<el-form-item label="<?=$label?>" <?=$item_attr?>>
    <table style="width:100%;" class="table table-bordered">
        ...
        <tbody>
            <tr v-for="(v,index) in <?=$model?:'form'?>.<?=$name?>" :key="index">
                <td>
                    <el-input placeholder="<?=lang('属性名如：温度')?>" v-model="v.name"></el-input>
                </td>
                <td>
                    <el-input placeholder="<?=lang('多值用,、或空格分隔如：标准冰、少冰、去冰、温、热')?>"
                        v-model="v.values"></el-input>
                </td>
                <td>
...
<?php 
$default = "[{name:'',values:''}]";
$vue->formData($name, $default);
 
$vue->method("form_push_attr(field)","
    if(!this.".$model."[field]) {
        this.\$set(this.".$model.", field, ".$default.");
    }
